...
public class ChefImpl implements Chef, BeltObserver {
	...
	@Override
	//method for calculating food consumed by customers on the belt
	public double foodConsumed(Plate p) {
		double amount = 0;
		if (p != null) {
			for (int i = 0; i < p.getContents().getIngredients().length; i++) {
				amount += p.getContents().getIngredients()[i].getAmount();
			}
		}
		return amount;
	}
	...
	@Override
	//method for calculating ounces of food spoiled on the belt
	public double foodSpoiled() {
		for (HistoricalPlate a : plate_history) {
			if (a.wasSpoiled()) {
				IngredientPortion[] ingredients = a.getContents().getIngredients();
				for (int i = 0; i < ingredients.length; i++) {
					spoiled_food += ingredients[i].getAmount();
				}
			}
		}
		return spoiled_food;

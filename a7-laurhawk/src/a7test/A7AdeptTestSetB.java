...
public class A7AdeptTestSetB {

	private final double DELTA = 0.005;

	private YellowtailPortion yellowtail = new YellowtailPortion(1.5);
	private SeaweedPortion seaweed = new SeaweedPortion(0.3);
	private RicePortion rice = new RicePortion(1.0);
	private IngredientPortion[] ingredients = new IngredientPortion[] { yellowtail,
			seaweed, rice };
	...
	public void testProfitCounterExistingBelt() {
		...
		assertEquals(15.02, counter.getTotalBeltProfit(), DELTA);
		assertEquals(1.50, counter.getAverageBeltProfit(), DELTA);
	}

	@Test
	public void testGetTotalBeltProfit() throws BeltPlateException {
		assertEquals(15.02, profitCounter.getTotalBeltProfit(), DELTA);

		belt.removePlateAtPosition(4);
		assertEquals(13.11, profitCounter.getTotalBeltProfit(), DELTA);

		belt.removePlateAtPosition(3);
		assertEquals(12.72, profitCounter.getTotalBeltProfit(), DELTA);

		belt.setPlateAtPosition(red, 3);
		assertEquals(13.11, profitCounter.getTotalBeltProfit(), DELTA);

		belt.setPlateAtPosition(green, 4);
		assertEquals(13.5, profitCounter.getTotalBeltProfit(), DELTA);
	}
	...
	public void testGetAverageBeltProfit() throws BeltPlateException {
		...
		belt.removePlateAtPosition(4);
		assertEquals(1.456, profitCounter.getAverageBeltProfit(), DELTA);
		...
		belt.setPlateAtPosition(green, 4);
		assertEquals(1.35, profitCounter.getAverageBeltProfit(), DELTA);
	}

	@Test
	public void testGetAverageBeltProfitZero() {
		Belt b = new BeltImpl(3);
